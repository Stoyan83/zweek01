        ...
        <div className="max-w-[1180px] lg:mt-32 lg:mb-[88px] gap-20 flex flex-wrap justify-start items-center max-sm:pb-12">
          ...
          {logos.map((image: any, index: number) => (
            <Image
              ...
              width={image.width}
              height={image.height}
              className={`max-sm:mx-4 max-sm:ml-12 ${index === 0 ? 'max-sm:-mr-3 ml-6 lg:-mr-1' : ''} ${index === 1 ? 'max-sm:-mr-3 max-sm:ml-12 lg:-mr-1' : ''} ${index === 2 ? 'lg:-ml-2 max-sm:mx-4 max-sm:ml-12' : ''} ${index === 3 ? 'lg:-ml-6 max-sm:-ml-6' : ''} ${index === 4 ? 'max-sm:mx-32 lg:-ml-5' : ''} max-sm:w-[94px] w-[${image.width}px] h-[${image.height}px]`}
            />
          ))}
        </div>
      );
